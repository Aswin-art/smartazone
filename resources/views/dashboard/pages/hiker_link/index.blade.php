        ...
        <div class="mb-3">
          ...
          <select id="bookingSelect" name="booking_id" class="form-select" required>
            ...
            @foreach($bookings as $b)
              <option value="{{ $b->id }}" data-members='@json($b->members)'>Booking #{{ $b->id }} - {{ $b->qr_code }} - team  {{ $b->team_size }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="mb-3">
          ...
          <select name="device_id" class="form-select" required>
            ...
            @foreach($devices as $d)
              <option value="{{ $d->id }}">Device #{{ $d->id }} (Battery {{ $d->battery_level ?? 'N/A' }}%)</option>
            @endforeach
          </select>
        </div>
      </div>
