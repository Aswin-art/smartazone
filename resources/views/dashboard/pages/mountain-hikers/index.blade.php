                ...
                <div class="modal-content border-0 shadow-lg rounded-3 overflow-hidden">
                    ...
                    <div class="modal-body p-0 position-relative">
                        <div id="mapContainer" style="width:100%;height:550px;">
                            ...
                            <div id="infoOverlay" class="position-absolute top-0 start-0 m-3 bg-white bg-opacity-75 p-3 rounded shadow-sm" style="z-index:999;">
...
<style>
#mapModal .modal-body { background-color: #f8f9fa; }
.ol-popup {
    position: absolute;
...
$(document).ready(function() {
    ...
    function loadHikers() {
        $('#hikerTable tbody').html('<tr><td colspan="6" class="text-center text-muted p-3">Memuat data...</td></tr>')
        $.ajax({
    ...
    function renderMap(logs) {
        if (mapInstance) { mapInstance.setTarget(null); mapInstance = null }
        if (logs.length === 0) {
        ...
        const features = logs.map(log => new ol.Feature({
            geometry: new ol.geom.Point(ol.proj.fromLonLat([parseFloat(log.longitude), parseFloat(log.latitude)])),
            data: log
        }))
        ...
        mapInstance = new ol.Map({
            ...
            view: new ol.View({
                center: ol.proj.fromLonLat([parseFloat(first.longitude), parseFloat(first.latitude)]),
                zoom: 10
            }),
            controls: []
        })
        ...
        mapInstance.getView().fit(extent, { size: mapInstance.getSize(), padding: [100, 100, 100, 100], duration: 800, maxZoom: 13 })

        const popup = document.createElement('div')
        popup.className = 'ol-popup'
        overlayPopup = new ol.Overlay({ element: popup, autoPan: true, autoPanAnimation: { duration: 250 } })
        ...
        mapInstance.on('click', function(evt) {
            ...
            if (feature) {
                ...
                popup.innerHTML = `<b>${d.timestamp}</b><br>❤️ BPM: ${d.heart_rate ?? '-'}<br>🩸 SpO₂: ${d.spo2 ?? '-'}<br>😌 Stres: ${d.stress_level ?? '-'}`
